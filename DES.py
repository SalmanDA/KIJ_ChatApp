...
class des():
    ...
    def run(self, key, text, action=ENCRYPT, padding=False):
        ...
        self.transform_keys()  # Generate all the keys
        # print(self.keys)
        ...
        for block in text_blocks:  # Loop over all the blocks of data
            ...
            for i in range(16):  # Do the 16 rounds
                ...
                l = r
                r = tmp
            result += self.permut(r + l, IP_1)  # Do the last permut and append the result to result
    ...
    def transform_keys(self):  # Algorithm that generates all the keys
        self.keys = []
        key = string_to_bit_array(self.password)
        key = self.permut(key, PC_1)  # Apply the initial permut on the key
        l, r = nsplit(key, 28)  # Split it in to (l->LEFT),(r->RIGHT)
        for i in range(16):  # Apply the 16 rounds
            l, r = self.shift(l, r, SHIFT[i])  # Apply the shift associated with the round (not always 1)
            tmp = l + r  # Merge them
            self.keys.append(self.permut(tmp, PC_2))  # Apply the permut to get the Ki

    def shift(self, l, r, n):  # Shift a list of the given value
